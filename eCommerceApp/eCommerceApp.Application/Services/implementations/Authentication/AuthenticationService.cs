    ...
    IValidator<LoginUser> loginUserValidator, IValidationService validationService) : IAuthenticationService
{
    public async Task<ServiceResponse> CreateUser(CreateUser user)
    {
        ...
        if (!result)
            return new ServiceResponse { Message = "Email Address might already be in use or unknown error occured!" };
        ...
        if (!assignedResult)
        {
            ...
            {
                logger.LogError(
                    new Exception($"User with Email as {_user.Email} failed to be removed as a result of role assinged "),"User could not be assigned Role");
